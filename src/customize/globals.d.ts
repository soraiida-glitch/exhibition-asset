declare const __WEBHOOK_URL__: string;
declare const __WEBHOOK_SECRET__: string;
declare const __ACCOUNT_APP_ID__: string;
declare const __OPPORTUNITY_APP_ID__: string;
declare const __LEAD_APP_ID__: string;
declare const __MEISHI_WEBHOOK_URL__: string;
declare const __CLOSING_ADVICE_WEBHOOK_URL__: string;
declare const __DAILY_ADVICE_APP_ID__: string;
declare const __ROLEPLAY_START_WEBHOOK_URL__: string;
declare const __ROLEPLAY_CHAT_WEBHOOK_URL__: string;
declare const __ROLEPLAY_FEEDBACK_WEBHOOK_URL__: string;
declare const __TRANSCRIBE_WEBHOOK_URL__: string;
declare const __TTS_WEBHOOK_URL__: string;
declare const __MEETING_LOG_APP_ID__: string;
declare const __MEETING_LOG_WEBHOOK_URL__: string;
declare const __ASSIGNEE_APP_ID__: string;
declare const __SALES_SCORE_APP_ID__: string;
declare const __SALES_SCORING_WEBHOOK_URL__: string;
declare const __PROPOSAL_WEBHOOK_URL__: string;
