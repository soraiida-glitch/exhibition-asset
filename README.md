...
- 「📋 取引先登録」「💼 案件登録」チップをクリック → 直接登録フォームが開く
  (業種・ステータス・フェーズはプルダウン選択)
...
│   ├── workflows/agent-workflow.ts  # 秘書AIエージェントのn8nノード/コネクション定義
│   ├── customize/chat.ts       # kintoneチャットUI(ブラウザ側、Viteでビルド)
│   └── scripts/
